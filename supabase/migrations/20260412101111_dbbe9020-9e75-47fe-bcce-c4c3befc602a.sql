...
DROP POLICY "Authenticated users can insert slides" ON public.hero_slides;
DROP POLICY "Authenticated users can update slides" ON public.hero_slides;
DROP POLICY "Authenticated users can delete slides" ON public.hero_slides;
...
CREATE POLICY "Admins can delete content" ON public.site_content FOR DELETE TO authenticated USING (public.has_role(auth.uid(), 'admin'));
